...
export default function SalesPage() {
  ...
  const allSaleGames = useMemo(() => {
    ...
    // 최소 할인율 필터
    const filtered = combined.filter((g) => g.discountPercent >= minDiscount);
    ...
    switch (sortBy) {
      ...
      case 'savings':
        return filtered.sort(
          (a, b) =>
            (b.originalPrice || 0) - b.finalPrice -
            ((a.originalPrice || 0) - a.finalPrice)
        );
      default:
        return filtered;
    }
  ...
  const stats = useMemo(() => {
    ...
    const maxDiscount = Math.max(...allSaleGames.map((g) => g.discountPercent));
    const avgDiscount = Math.round(
      allSaleGames.reduce((sum, g) => sum + g.discountPercent, 0) / allSaleGames.length
    );
    const totalSavings = allSaleGames.reduce(
      (sum, g) => sum + ((g.originalPrice || 0) - g.finalPrice),
      0
    );
  ...
  const discountDistribution = useMemo(() => {
    ...
    allSaleGames.forEach((g) => {
      if (g.discountPercent >= 75) dist.over75++;
      else if (g.discountPercent >= 50) dist.over50++;
      else if (g.discountPercent >= 25) dist.over25++;
      else dist.under25++;
    });
    return dist;
  }, [allSaleGames]);
            ...
            <div className="space-y-3">
              {bestDeals.map((game, index) => (
                <div key={game.appId} className="flex items-center justify-between">
                  ...
                  <Badge className="bg-green-600">-{game.discountPercent}%</Badge>
                </div>
              ))}
            </div>
          </CardContent>
                  ...
                  <TableRow key={game.appId}>
                    ...
                    <TableCell className="text-right">
                      <span className="flex items-center justify-end gap-1 text-orange-500">
                        ...
                        {formatPrice((game.originalPrice || 0) - game.finalPrice, game.currency)}
                      </span>
                    </TableCell>
