...
import {
  generateGameInsight,
  generateGameUpdateSummary,
  generateGameStandardizedInsight,
  isGeminiConfigured
} from '@/lib/api/gemini';
...
export async function POST(
  ...
  try {
    ...
    let insight: string | StandardizedInsight;

    if (requestType === 'update_summary') {
      // 최근 동향 요약 (뉴스 포함) - 레거시 텍스트 형식
      ...
      // 일반 게임 인사이트 - 구조화된 형식 (projections, comparables 포함)
      console.log('[Game Insight] Generating standardized insight for', body.name);
      insight = await generateGameStandardizedInsight({
        appId: parseInt(appId),
        name: body.name,
