...
const SENTIMENT_KEYWORDS = {
  positive: {
    ko: [
      ...
      '갓겜', '꿀잼', '핵꿀잼', '레전드', '킹받', '존잼', '미침', '개꿀', '필수',
      '인생게임', '역대급', '띵작', '꿀팁', '핵이득', '개추', '씹덕', '미쳤', '대성공',
      '아름다', '감탄', '빠져든', '몰입', '소름', '인정', '존버', '갓작', '예술',
      '필플', '해야할', '해볼만', '무조건', '짱', '굿', '쩔', '괜찮', '나쁘지',
    ],
    en: [
      'amazing', 'awesome', 'great', 'excellent', 'love', 'best', 'recommend',
      'masterpiece', 'fantastic', 'perfect', 'incredible', 'must-play', 'addictive',
      'innovative', 'polished', 'satisfying', 'beautiful', 'fun', 'enjoy', 'worth',
      'underrated', 'gem', 'favorite', 'stunning', 'impressed', 'brilliant', 'solid',
      'smooth', 'engaging', 'immersive', 'compelling', 'must buy', 'must have',
      'good', 'nice', 'cool', 'epic', 'legendary', 'phenomenal', 'outstanding',
      'remarkable', 'exceptional', 'superb', 'magnificent', 'wonderful', 'impressive',
      '10/10', '9/10', '5/5', 'recommended', 'worth it', 'buy it', 'get it',
    ],
  },
  negative: {
    ko: [
      ...
      '핵노잼', '똥겜', '폭망', '쓰렉', '거르', '비추천', '돈아까', '망한', '실패',
      '문제', '오류', '에러', '끊김', '튕김', '프레임', '드랍', '최적화', '안됨',
      '노답', '헬', '극혐', '하지마', '사지마', '패스', '버림', '삭제', '지움',
    ],
    en: [
      'boring', 'disappointing', 'bad', 'worst', 'terrible', 'awful', 'waste',
      'buggy', 'broken', 'laggy', 'refund', 'regret', 'frustrating', 'annoying',
      'overpriced', 'unfinished', 'repetitive', 'shallow', 'generic', 'mediocre',
      'skip', 'avoid', 'not worth', 'don\'t buy', 'disappointed', 'failed', 'failure',
      'trash', 'garbage', 'horrible', 'poor', 'weak', 'lacking', 'missing',
      'problems', 'issues', 'bugs', 'crash', 'crashes', 'unstable', 'unplayable',
      'dead', 'died', 'dying', 'empty', 'abandoned', 'scam', 'ripoff', 'cash grab',
      '1/10', '2/10', '3/10', '0/10', 'not recommended', 'skip it', 'pass',
    ],
  },
};
...
export function analyzeVideoSentiment(
  title: string,
  description: string,
  likeRatio?: number // 좋아요 / 조회수 비율 (0-1)
): VideoSentiment {
  ...
  // 긍정 키워드 체크 (제목에 있으면 가중치 2배)
  for (const keyword of SENTIMENT_KEYWORDS.positive.ko) {
    if (titleLower.includes(keyword.toLowerCase())) {
      positiveScore += 15; // 제목에 있으면 더 높은 점수
    } else if (text.includes(keyword.toLowerCase())) {
      positiveScore += 8;
    }
  }
  for (const keyword of SENTIMENT_KEYWORDS.positive.en) {
    if (titleLower.includes(keyword.toLowerCase())) {
      positiveScore += 15;
    } else if (text.includes(keyword.toLowerCase())) {
      positiveScore += 8;
  ...
  // 부정 키워드 체크
  for (const keyword of SENTIMENT_KEYWORDS.negative.ko) {
    if (titleLower.includes(keyword.toLowerCase())) {
      negativeScore += 15;
    } else if (text.includes(keyword.toLowerCase())) {
      negativeScore += 8;
  ...
  for (const keyword of SENTIMENT_KEYWORDS.negative.en) {
    if (titleLower.includes(keyword.toLowerCase())) {
      negativeScore += 15;
    } else if (text.includes(keyword.toLowerCase())) {
      negativeScore += 8;
  ...
  // 리뷰 영상 특성 반영 - "리뷰"가 제목에 있고 키워드가 없으면 기본 긍정으로 간주
  // (대부분의 리뷰 영상은 추천 목적으로 만들어짐)
  if (positiveScore === 0 && negativeScore === 0) {
    if (titleLower.includes('리뷰') || titleLower.includes('review')) {
      positiveScore += 10; // 기본 긍정 점수
    }
    if (titleLower.includes('추천') || titleLower.includes('recommend')) {
      positiveScore += 15;
    }
    if (titleLower.includes('플레이') || titleLower.includes('gameplay') || titleLower.includes('let\'s play')) {
      positiveScore += 5; // 플레이 영상은 약한 긍정
    }
  }

  // 좋아요 비율 반영 (있는 경우)
  if (likeRatio !== undefined) {
    if (likeRatio > 0.05) { // 5% 이상 좋아요 비율은 긍정 신호
      positiveScore += Math.round(likeRatio * 100);
    } else if (likeRatio < 0.01) { // 1% 미만은 부정 신호
      negativeScore += 10;
    }
  }
  ...
  if (positiveScore > negativeScore + 10) {
    ...
    overall = 'negative';
  } else if (positiveScore > 15 && negativeScore > 15) {
    overall = 'mixed';
  ...
  const confidence = Math.min(1, totalSignals / 40);
...
export function analyzeVideos(
  ...
  return videos.map(video => {
    // 좋아요/조회수 비율 계산
    const likeRatio = video.viewCount > 0 ? video.likeCount / video.viewCount : undefined;

    return {
      ...video,
      sentiment: analyzeVideoSentiment(video.title, video.description, likeRatio),
